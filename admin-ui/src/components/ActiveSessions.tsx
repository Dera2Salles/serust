...
export const ActiveSessions = () => {
  ...
  const handleKick = async (sessionId: string) => {
    try {
      ...
      await fetch(`http://localhost:8081/api/server/sessions/${sessionId}`, {
        method: "DELETE",
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      });
      // Refresh sessions list after kick
      fetchStatus();
    } catch (e) {
      ...
      <div className="px-8 grid grid-cols-1 gap-5">
        {loading && sessions.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 opacity-40">
            ...
            <p className="text-[--color-win-text3] font-semibold uppercase text-[10px] tracking-widest">
              Écoute du port 8081...
            </p>
          </div>
        ) : sessions.length === 0 ? (
          <div className="fluent-card flex flex-col items-center justify-center py-32 border-dashed border-2 border-[--color-win-stroke]  shadow-none opacity-50">
            ...
            <h4 className="text-xl font-semibold text-[--color-win-text] mb-1 tracking-tight">
              Aucune activité réseau
            </h4>
            <p className="text-sm font-medium text-[--color-win-text3]">
          ...
          sessions.map((session) => (
            <div
              key={session.id}
              className="fluent-card flex items-center justify-between group transition-all py-8 bg-white shadow-sm border border-[--color-win-stroke]/50"
            >
              <div className="flex items-center gap-7">
                ...
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    <h4 className="font-semibold text-xl tracking-tighter text-[--color-win-text]">
                      {session.username || "Client Anonyme"}
                    </h4>
                    {session.username ? (
                  ...
                  <div className="flex flex-wrap items-center gap-6 text-sm text-[--color-win-text3] font-medium">
                    ...
                    {session.last_command && (
                      <span className="flex items-center gap-2">
                        <Activity
                          size={14}
                          className="text-[--color-accent] opacity-50"
                        />
                        Action :{" "}
                        <span className="font-semibold text-[--color-win-text] tracking-tight uppercase text-xs bg-[--color-accent]/5 px-2 py-0.5 rounded">
                          {session.last_command}
                        </span>
                      </span>
                    )}
              ...
              <div className="flex items-center gap-3">
                <button
                  className="px-6 py-3 bg-[--color-error]/5 text-[--color-error] hover:bg-[--color-error] hover:text-white font-semibold text-[10px] uppercase tracking-widest rounded-full transition-all active:scale-95 border border-red/10 shadow-sm"

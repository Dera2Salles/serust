...
export const Login: React.FC<LoginProps> = ({ onLoginSuccess }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const user = await invoke<any>("login_admin_db", {
        email,
        passwordRaw: password,
      });

      // Admin sessions are never persisted to localStorage for security reasons.
  ...
  return (
    <div
      ...
      style={{
        ...
        backgroundImage:
          "radial-gradient(circle at 50% 50%, var(--color-accent-subtle) 0%, transparent 70%)",
        opacity: 0.95,
      }}
    >
      <div
        className="fluent-card w-full max-w-md p-10 animate-in fade-in zoom-in duration-500 shadow-2xl"
        style={{
          borderRadius: 20,
          ...
          <div className="flex items-center justify-between">
            <label className="flex items-center gap-2 cursor-pointer group">
              ...
              <span
                ...
                className="group-hover:text-[--color-accent] transition-colors"
              >
                Se souvenir de moi
              </span>
            </label>

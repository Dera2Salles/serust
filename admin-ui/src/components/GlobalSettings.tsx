...
import { Shield, Save, RefreshCcw, Lock, Server, Globe, Zap, RotateCcw } from 'lucide-react';
...
const SettingRow = ({ label, desc, checked, onChange, danger = false }: { label: string; desc: string; checked: boolean; onChange: () => void; danger?: boolean }) => (
  ...
  >
    <div style={{ flex: 1, paddingRight: 16 }}>
      ...
      <p style={{ fontSize: 12, color: 'var(--color-win-text3)', margin: 0, lineHeight: 1.4 }}>{desc}</p>
    </div>
    <div 
...
export const GlobalSettings: React.FC = () => {
  ...
  const handleReset = async () => {
    if (!window.confirm("Voulez-vous vraiment réinitialiser tous les paramètres aux valeurs par défaut ?")) return;
    ...
    setMessage({ text: 'Paramètres réinitialisés (n\'oubliez pas de sauvegarder)', type: 'success' });
  };
  ...
  return (
    <div style={{ paddingBottom: 60 }}>
      <Header title="Paramètres Système" subtitle="Gérez les politiques de sécurité, les quotas et les ports réseau de votre serveur Kajy." />

      <div style={{ padding: '0 28px', display: 'grid', gridTemplateColumns: '1fr 320px', gap: 24, maxWidth: 1100 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div className="fluent-card" style={{ padding: '24px' }}>
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <p style={{ fontWeight: 700, fontSize: 16, margin: 0 }}>Sécurité et Accès</p>
                <p style={{ fontSize: 12, color: 'var(--color-win-text3)', margin: 0 }}>Contrôlez qui peut accéder à votre serveur</p>
              </div>
            </div>
          ...
          <div className="fluent-card" style={{ padding: '24px' }}>
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <p style={{ fontWeight: 700, fontSize: 16, margin: 0 }}>Ressources et Quotas</p>
                <p style={{ fontSize: 12, color: 'var(--color-win-text3)', margin: 0 }}>Limites de stockage et de transfert</p>
              </div>
            </div>
            
            <div className="grid grid-cols-2 gap-6">
              <div className="flex flex-col gap-2">
                <label style={{ fontSize: 12, fontWeight: 700, color: 'var(--color-win-text2)', display: 'flex', alignItems: 'center', gap: 6 }}>
                  <Zap size={14} style={{ color: 'var(--color-warning)' }} /> Quota par défaut (GB)
                </label>
                <input
                  type="number"
                  ...
                  onChange={e => settings && setSettings({ ...settings, default_storage_quota_gb: Math.max(0, Number(e.target.value)) })}
                />
                <p style={{ fontSize: 11, color: 'var(--color-win-text4)' }}>Espace alloué aux nouveaux utilisateurs.</p>
              </div>
              <div className="flex flex-col gap-2">
                <label style={{ fontSize: 12, fontWeight: 700, color: 'var(--color-win-text2)', display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <input
                  ...
                  onChange={e => settings && setSettings({ ...settings, max_upload_size_mb: Math.max(1, Number(e.target.value)) })}
                />
                <p style={{ fontSize: 11, color: 'var(--color-win-text4)' }}>Limite par fichier lors du transfert.</p>
              </div>
            </div>
          </div>
          ...
          <div className="fluent-card" style={{ padding: '24px' }}>
            <div className="flex items-center gap-3 mb-6">
              <div style={{ width: 36, height: 36, borderRadius: 10, background: 'var(--color-accent-subtle)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Globe size={18} style={{ color: 'var(--color-accent)' }} />
              </div>
              <div>
                <p style={{ fontWeight: 700, fontSize: 16, margin: 0 }}>Ports et Réseau</p>
                <p style={{ fontSize: 12, color: 'var(--color-win-text3)', margin: 0 }}>Configuration des services d'écoute</p>
              </div>
            </div>
            
            <div className="grid grid-cols-3 gap-4">
              <div className="flex flex-col gap-2">
                <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--color-win-text3)', textTransform: 'uppercase' }}>Port MCP</label>
              ...
              <div className="flex flex-col gap-2">
                <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--color-win-text3)', textTransform: 'uppercase' }}>Port WebDAV</label>
              ...
              <div className="flex flex-col gap-2">
                <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--color-win-text3)', textTransform: 'uppercase' }}>Port S3</label>
            ...
            <div style={{ 
              marginTop: 20, padding: '10px 14px', borderRadius: 8, background: 'var(--color-win-bg)', 
              borderLeft: '4px solid var(--color-accent)', display: 'flex', alignItems: 'center', gap: 10
            }}>
              <Server size={16} style={{ color: 'var(--color-accent)' }} />
              <p style={{ fontSize: 12, color: 'var(--color-win-text3)', margin: 0 }}>
                <strong>Note:</strong> Un redémarrage manuel du serveur est nécessaire pour appliquer ces ports.
              </p>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div className="fluent-card" style={{ background: 'var(--color-accent)', border: 'none', boxShadow: 'var(--shadow-16)', color: 'white', padding: '24px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
              <Save size={20} />
              <p style={{ fontWeight: 700, fontSize: 16, margin: 0 }}>Sauvegarder</p>
            </div>
            <p style={{ fontSize: 13, opacity: 0.9, marginBottom: 20, lineHeight: 1.5 }}>
              Appliquer les modifications immédiatement sur le fichier de configuration global.
            </p>
            <button
              onClick={handleSave}
              disabled={saving || !settings}
              style={{
                width: '100%', padding: '10px 0', borderRadius: 8, border: '1px solid rgba(255,255,255,0.2)',
            ...
            {message && (
              <div style={{
                ...
                background: message.type === 'success' ? 'rgba(255,255,255,0.2)' : 'rgba(220,38,38,0.5)',
          ...
          <div className="fluent-card" style={{ padding: '20px' }}>
            <div className="flex items-center gap-2 mb-4">
              <RotateCcw size={16} style={{ color: 'var(--color-win-text3)' }} />
              <p style={{ fontWeight: 700, fontSize: 14, margin: 0 }}>Actions rapides</p>
            </div>
            <button 
          ...
          <div className="fluent-card" style={{ padding: '20px', background: 'var(--color-win-nav)' }}>
            <p style={{ fontWeight: 700, fontSize: 13, marginBottom: 10 }}>Statut Config</p>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div style={{ width: 8, height: 8, borderRadius: '50%', background: settings ? '#107c10' : '#888' }} />
              <span style={{ fontSize: 12, color: 'var(--color-win-text2)' }}>
                {settings ? 'Fichier chargé' : 'Erreur de chargement'}
              </span>
            </div>
            <p style={{ fontSize: 11, color: 'var(--color-win-text4)', marginTop: 12 }}>

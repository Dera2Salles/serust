...
export const UserManagement = () => {
  ...
  const fetchDefaultQuota = async () => {
    try {
      ...
      if (settings?.default_storage_quota_gb) {
        setDefaultQuota(settings.default_storage_quota_gb);
        setFormData((prev) => ({
          ...prev,
          quota_gb: settings.default_storage_quota_gb,
        }));
      }
    } catch (e) {
  ...
  const handleApprove = async (user: User) => {
    try {
      ...
      alert("Erreur lors de l'approbation : " + e);
    }
  };
    ...
    <div style={{ paddingBottom: 48 }}>
      ...
      >
        ...
        >
          <Search
            size={14}
            style={{ color: "var(--color-win-text3)", flexShrink: 0 }}
          />
          <input
            style={{
              border: "none",
            ...
            <div className="flex items-center justify-center py-16">
              <div
                style={{
                  width: 24,
                  height: 24,
                  border: "2px solid var(--color-win-stroke2)",
            ...
            filtered.map((user, i) => {
              try {
                ...
                const usedPct = Math.min(
                  100,
                  (usedBytes / Math.max(quotaBytes, 1)) * 100,
                );

                let dateStr = "N/A";
                if (user.created_at) {
                  try {
                    dateStr = new Date(user.created_at).toLocaleDateString(
                      "fr-FR",
                      { day: "2-digit", month: "short", year: "numeric" },
                    );
                  } catch (de) {
                    console.error("Date error", de);
                  }
                }
                  ...
                  <div key={user.id || i}>
                    {i > 0 && (
                      <div
                        style={{
                          height: 1,
                          background: "var(--color-win-stroke)",
                          margin: "0 16px",
                        }}
                      />
                    )}
      ...
      {isCreateOpen && (
        <div
          className="fluent-dialog-overlay"
          onClick={(e) => {
        ...
        >
          <div className="fluent-dialog">
            ...
            >
              <div>
                <p className="fluent-dialog-title" style={{ margin: 0 }}>
                  Nouvel utilisateur
                </p>
                <p
                  style={{
                ...
                >
                  Le compte sera créé en attente d'approbation
                </p>
              </div>
              <button
                className="fluent-btn"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      quota_gb: Number(e.target.value),
                    })
                  }
                  required
                />

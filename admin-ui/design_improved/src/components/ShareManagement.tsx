...
import { 
  Share2, Link as LinkIcon, User, Trash2, Calendar, 
  RefreshCw, ExternalLink, ShieldCheck, 
  Search, Copy, CheckCircle2, AlertCircle, Clock
} from 'lucide-react';
...
export const ShareManagement = () => {
  ...
  const copyToken = (share: Share) => {
    if (!share.token) return;
    navigator.clipboard.writeText(share.token);
    setCopyId(share.id);
  ...
  const stats = {
    ...
    expired: shares.filter(s => (s.expires_at && new Date(s.expires_at) < new Date()) || s.is_active === false).length
  };
      ...
      <div className="px-8 mb-8 grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="fluent-stat">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-semibold uppercase tracking-wider text-[--color-win-text3]">Total Partages</span>
            <Share2 size={14} className="text-[--color-accent]" />
          </div>
          <p className="text-2xl font-bold text-[--color-win-text]">{stats.total}</p>
        </div>
        <div className="fluent-stat">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-semibold uppercase tracking-wider text-[--color-win-text3]">Liens Publics</span>
            <LinkIcon size={14} className="text-[--color-accent]" />
          </div>
          <p className="text-2xl font-bold text-[--color-win-text]">{stats.links}</p>
        </div>
        <div className="fluent-stat">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-semibold uppercase tracking-wider text-[--color-win-text3]">Accès Directs</span>
            <User size={14} className="text-[--color-accent]" />
          </div>
          <p className="text-2xl font-bold text-[--color-win-text]">{stats.grants}</p>
        </div>
        <div className="fluent-stat">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-semibold uppercase tracking-wider text-[--color-win-text3]">Inactifs / Expirés</span>
            <Clock size={14} className="text-[--color-error]" />
          </div>
          <p className="text-2xl font-bold text-[--color-error]">{stats.expired}</p>
        </div>
      </div>
      ...
      <div className="px-8 space-y-3">
        {loading && shares.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 opacity-40">
             ...
             <p className="text-xs font-bold uppercase tracking-widest text-[--color-win-text3]">Chargement des partages...</p>
          </div>
          ...
          <div className="fluent-card flex flex-col items-center justify-center py-24 border-dashed border-2 opacity-50">
            ...
            <p className="text-[--color-win-text] font-semibold">Aucun partage ne correspond à votre recherche</p>
          </div>
        ) : (
            ...
            return (
              <div key={share.id} className={cn(
                ...
                <div className={cn(
                  ...
                  isLink ? "bg-[--color-accent-subtle] text-[--color-accent]" : "bg-[--color-success-bg] text-[--color-success]"
                )}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className={cn(
                      ...
                      isLink ? "bg-[--color-accent] text-white" : "bg-[--color-win-nav] text-[--color-win-text2]"
                    )}>
                  ...
                  <div className="flex flex-wrap items-center gap-x-5 gap-y-1 text-[12px] text-[--color-win-text3]">
                    <span className="flex items-center gap-1.5">
                      <ShieldCheck size={13} className="text-[--color-accent] opacity-60" />
                      De <span className="font-bold text-[--color-win-text2]">{share.owner}</span>
                    </span>
                    <span className="flex items-center gap-1.5">
                      <ExternalLink size={13} className="opacity-40" />
                      Pour <span className="font-bold text-[--color-win-text2]">{share.grantee}</span>
                    </span>
                    {share.expires_at && (
                      <span className="flex items-center gap-1.5">
                        <Calendar size={13} className="opacity-40" />
                        Expire : {new Date(share.expires_at).toLocaleDateString()}
                      </span>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-center gap-6">
                  <div className="hidden xl:flex flex-col items-end gap-1">
                    <span className="text-[9px] font-bold uppercase tracking-widest opacity-40">Permissions</span>
                    <div className="flex gap-1">
                      <div className={cn("w-2 h-2 rounded-full", share.can_read ? "bg-[--color-success]" : "bg-[--color-error]")} title="Lecture" />
                      <div className={cn("w-2 h-2 rounded-full", share.can_write ? "bg-[--color-warning]" : "bg-[--color-error]")} title="Écriture" />
                    </div>
                  </div>
                  ...
                  {isLink && (
                    ...
                    >
                      {copiedId === share.id ? <CheckCircle2 size={14} className="text-[--color-success]" /> : <Copy size={14} />}
                      <span className="text-[11px] font-bold uppercase tracking-tighter">Token</span>
                    </button>
                  )}

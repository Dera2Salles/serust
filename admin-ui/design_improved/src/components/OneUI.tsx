...
export const Header = ({ title, subtitle, className }: HeaderProps) => (
  <div className={cn('px-7 pt-7 pb-5', className)}>
    <p style={{ fontSize: 11, fontWeight: 600, color: 'var(--color-accent)', textTransform: 'uppercase', letterSpacing: '0.08em', margin: '0 0 4px' }}>
      Kajy Admin
    </p>
    <h1 style={{ fontSize: 24, fontWeight: 700, color: 'var(--color-win-text)', letterSpacing: '-0.4px', lineHeight: 1.2, margin: 0 }}>
      {title}
    </h1>
    {subtitle && (
      <p style={{ fontSize: 13, color: 'var(--color-win-text3)', marginTop: 4, marginBottom: 0 }}>{subtitle}</p>
    )}
  </div>
);
...
export const SectionDivider = ({ label }: SectionDividerProps) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: '8px 0' }}>
    <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--color-win-text3)', textTransform: 'uppercase', letterSpacing: '0.07em', whiteSpace: 'nowrap' }}>{label}</span>
    <div style={{ flex: 1, height: 1, background: 'var(--color-win-stroke)' }} />
  </div>
);

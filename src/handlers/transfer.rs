...
impl Handler for StorHandler {
    ...
    async fn handle(
        ...
        let mut data_stream = match get_data_stream(ctx).await {
            Ok(s) => s,
            Err(e) => { error!("STOR data connect: {}", e); ctx.error(425, "Can't open data connection."); return Ok(()); }
        };
        ...
        match self.files.upload(&user, &cwd, filename, size, data).await {
            ...
            Err(e) => { error!("STOR upload: {}", e); ctx.error(451, "Requested action aborted. Local error in processing."); }
        }
        Ok(())
    }
}
...
impl Handler for RetrHandler {
    ...
    async fn handle(
        ...
        let data = match self.files.download(&user, &cwd, filename).await {
            Ok(d) => d,
            Err(e) => { error!("RETR: {}", e); ctx.error(550, "Requested action not taken. File unavailable."); return Ok(()); }
        };
        ...
        let mut data_stream = match get_data_stream(ctx).await {
            Ok(s) => s,
            Err(e) => { error!("RETR data connect: {}", e); ctx.error(425, "Can't open data connection."); return Ok(()); }
        };
...
impl Handler for ListDirHandler {
    ...
    async fn handle(
        ...
        for (name, is_dir) in &entries {
            ...
            let line = format!("{}{} 1 ftp ftp {:>10} Jan 01 00:00 {}\r\n",
                type_char, perms,
...
impl Handler for NlstHandler {
    fn command(&self) -> &'static str { "NLST" }
    async fn handle(
        &self, ctx: &mut Context, _: &[&str],
        _: &mut BufReader<tokio::net::tcp::ReadHalf<'_>>,
        writer: &mut tokio::net::tcp::WriteHalf<'_>,
    ) -> HandlerResult {
        let user = make_user(ctx);
...
impl Handler for MkdHandler {
    ...
    async fn handle(
        ...
        match self.files.mkdir(&user, &cwd, dirname).await {
            ...
            Err(e) => { error!("MKD: {}", e); ctx.error(550, "Failed to create directory."); }
        }
        Ok(())
    }
}
...
impl Handler for RmdHandler {
    ...
    async fn handle(
        ...
        match self.files.rmdir(&user, &cwd, dirname).await {
            ...
            Err(e) => { error!("RMD: {}", e); ctx.error(550, "Failed to remove directory."); }
        }
        Ok(())
    }
}
...
impl Handler for DeleHandler {
    ...
    async fn handle(
        ...
        match self.files.delete_file(&user, &cwd, filename).await {
            ...
            Err(e) => { error!("DELE: {}", e); ctx.error(550, "File not found or deletion failed."); }
        }
        Ok(())
    }
}
...
impl Handler for RnfrHandler {
    fn command(&self) -> &'static str { "RNFR" }
    async fn handle(
        &self, ctx: &mut Context, args: &[&str],
        _: &mut BufReader<tokio::net::tcp::ReadHalf<'_>>,
        _: &mut tokio::net::tcp::WriteHalf<'_>,
...
pub struct RntoHandler { files: Arc<FileService> }
impl RntoHandler { pub fn new(files: Arc<FileService>) -> Self { Self { files } } }

#[async_trait]
impl Handler for RntoHandler {
    fn command(&self) -> &'static str { "RNTO" }
    async fn handle(
        &self, ctx: &mut Context, args: &[&str],
        _: &mut BufReader<tokio::net::tcp::ReadHalf<'_>>,
        _: &mut tokio::net::tcp::WriteHalf<'_>,
        ...
        let from = match ctx.extensions.get::<RenameFrom>() {
            Some(r) => r.0.clone(),
            None => { ctx.error(503, "Bad sequence of commands. Send RNFR first."); return Ok(()); }
        };
        if args.is_empty() {
            ctx.error(501, "Syntax error in parameters or arguments.");
            return Ok(());
        ...
        match self.files.download(&user, &cwd, &from).await {
            Ok(data) => {
                ...
                match self.files.upload(&user, &cwd, to, size, data).await {
                    ...
                    Err(e) => { error!("RNTO upload: {}", e); ctx.error(553, "Rename failed."); }
                }
            }
            Err(e) => { error!("RNTO download: {}", e); ctx.error(550, "Source file not found."); }
        }
        Ok(())
    }
}
...
impl Handler for SizeHandler {
    fn command(&self) -> &'static str { "SIZE" }
    async fn handle(
        &self, ctx: &mut Context, args: &[&str],
        _: &mut BufReader<tokio::net::tcp::ReadHalf<'_>>,
        _: &mut tokio::net::tcp::WriteHalf<'_>,

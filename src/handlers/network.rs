...
impl Handler for PasvHandler {
    fn command(&self) -> &'static str { "PASV" }
    async fn handle(&self, ctx: &mut Context, _: &[&str], _: &mut BufReader<tokio::net::tcp::ReadHalf<'_>>, _: &mut tokio::net::tcp::WriteHalf<'_>) -> HandlerResult {
        match TcpListener::bind("0.0.0.0:0").await {
            Ok(listener) => {
                ...
                ctx.write_line(&format!("227 Entering Passive Mode ({},{},{}).", ip_str, p1, p2));
            }
            Err(e) => {
...
impl Handler for PortHandler {
    fn command(&self) -> &'static str { "PORT" }
    async fn handle(&self, ctx: &mut Context, args: &[&str], _: &mut BufReader<tokio::net::tcp::ReadHalf<'_>>, _: &mut tokio::net::tcp::WriteHalf<'_>) -> HandlerResult {
        if args.is_empty() {
